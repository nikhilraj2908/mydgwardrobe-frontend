...
export default function AddWardrobe() {
  ...
  const handleSubmit = async () => {
    ...
    if (!image || !finalCategory || !finalWardrobe) {
      Alert.alert("Error", "Image, category & wardrobe are required");
      return;
    }
    ...
    try {
      ...
      if (Platform.OS === "web" && image.uri.startsWith('blob:')) {
        ...
        const file = new File([blob], `wardrobe_${Date.now()}.jpg`, {
          type: image.mimeType || "image/jpeg",
        });
        formData.append("image", file);
      } else {
        // Mobile or web with regular URL
        formData.append("image", {
          uri: image.uri,
          name: image.fileName || `wardrobe_${Date.now()}.jpg`,
          type: image.mimeType || "image/jpeg",
        } as any);
      }
              ...
              <FlatList
                ...
                ListEmptyComponent={
                  <View style={styles.emptyList}>
                    ...
                    <Text style={styles.emptyListText}>No wardrobes yet</Text>
                    <Text style={styles.emptyListSubText}>
                      Select "Other" to create a new one
                    </Text>
                  </View>
                }
              />
